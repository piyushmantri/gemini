...
- Optional image uploader for Gemini vision models; attaches uploads as inline data in the prompt
...
3. Paste your Gemini API key, enter the full model ID (for example `models/gemini-3-pro-image-preview`), attach an optional image, and start chatting. Imagen models (e.g. `models/imagen-4.0-fast-generate-001`) will yield inline images; Gemini models respond with text.
...
## Usage Tips
- You can swap models at any time between turns—type a new `models/...` identifier into the field. Imagen-series models render generated images directly in the chat, while Gemini vision models can accept both text and uploaded images.
...
- Availability varies by account; some models (such as `models/nonobanana-3`) require special access and will return a 404 until enabled. Image uploads require a Gemini vision-capable model (for example, `models/gemini-3-pro-image-preview`).
